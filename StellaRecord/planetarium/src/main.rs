...
// §310 / §6.5 Regex の static 化による高速化
static RE_TIME: Lazy<Regex> = Lazy::new(|| Regex::new(r"^(\d{4}\.\d{2}\.\d{2} \d{2}:\d{2}:\d{2})").unwrap());
static RE_USER_AUTH: Lazy<Regex> = Lazy::new(|| Regex::new(r"User Authenticated: (.*?) \((usr_.*?)\)").unwrap());
static RE_BUILD: Lazy<Regex> = Lazy::new(|| Regex::new(r"VRChat Build: (.*)").unwrap());
static RE_ENTERING: Lazy<Regex> = Lazy::new(|| Regex::new(r"\[Behaviour\] Entering Room: (.*)").unwrap());
static RE_JOINING: Lazy<Regex> = Lazy::new(|| Regex::new(
    r"\[Behaviour\] Joining (wrld_[^:]+)(?::(\d+))?~?((?:private|friends|hidden|public|group)[^~]*)(?:~region\(([^)]+)\))?"
).unwrap());
static RE_LEFT_ROOM: Lazy<Regex> = Lazy::new(|| Regex::new(r"\[Behaviour\] OnLeftRoom").unwrap());
static RE_PLAYER_JOIN: Lazy<Regex> = Lazy::new(|| Regex::new(r"\[Behaviour\] OnPlayerJoined (.*?) \((usr_.*?)\)").unwrap());
static RE_PLAYER_LEFT: Lazy<Regex> = Lazy::new(|| Regex::new(r"\[Behaviour\] OnPlayerLeft (.*?) \((usr_.*?)\)").unwrap());
static RE_IS_LOCAL: Lazy<Regex> = Lazy::new(|| Regex::new(r#"\[Behaviour\] Initialized PlayerAPI "(.*?)" is (local|remote)"#).unwrap());
static RE_AVATAR: Lazy<Regex> = Lazy::new(|| Regex::new(r"\[Behaviour\] Switching (.*?) to avatar (.*)").unwrap());
static RE_VIDEO: Lazy<Regex> = Lazy::new(|| Regex::new(r"\[(?:<[^>]+>)?USharpVideo(?:</[^>]+>)?\] Started video load for URL: (.*?), requested by (.*)").unwrap());
static RE_VIDEO_ALT: Lazy<Regex> = Lazy::new(|| Regex::new(r"\[(?:<[^>]+>)?USharpVideo(?:</[^>]+>)?\] Started video: (.*)").unwrap());
static RE_USR: Lazy<Regex> = Lazy::new(|| Regex::new(r"\((usr_[^)]+)\)").unwrap());
...
fn parse_and_import(
    ...
    // §899 / §6.5 Windows SharedRead モードでのログアクセス実装 (デッドロック回避)
    #[cfg(windows)]
    let file = {
    ...
    // パフォーマンス問題3: 行数を事前にカウントして進捗率を表示？
    // 面倒なので、行のオフセットまたはファイルサイズベースで進捗を出すことも検討されるが、
    // ここではシンプルに「処理開始」を明示。
    ...
    for line_result in reader.lines() {
        ...
        if let Some(caps) = RE_PLAYER_JOIN.captures(&line) {
            ...
            // Privacy Mode
            if !tracking {
                // 自分自身の名前と一致するかチェック（雑な判定だが、Auth行で [LocalPlayer] になってるはず）
        ...
        if let Some(caps) = RE_VIDEO.captures(&line) {
            if let Some(vid) = current_visit_id {
                ...
                ).map_err(|e| { eprintln!("video_playbacks Error: {}", e); e })?;
            }
            continue;
        }
