...
fn main() {
    // 完全にバックグラウンドで動作させるため、コンソールから切り離す
    #[cfg(windows)]
    unsafe {
        extern "system" {
            fn FreeConsole() -> i32;
...
fn sync_logs() {
    ...
    // バックアップ先ディレクトリ: %LOCALAPPDATA%\CosmoArtsStore\STELLARECORD\app\Polaris\backup\
    let Ok(local_appdata) = std::env::var("LOCALAPPDATA") else { return };
    let dest_dir = Path::new(&local_appdata).join("CosmoArtsStore\\STELLARECORD\\app\\Polaris\\archive");
